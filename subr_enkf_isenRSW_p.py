...
from f_enkf_isenRSW import analysis_step_enkf_v4, analysis_step_enkf_v5
...
def run_enkf(i, j, m, l, U_tr_array, Y_obs, dirname, config_file,h5_file_data):
    ...
    while tmeasure-dtmeasure < t_end_assim and index < Nmeas:
        ...
        try:
            ...
            U_an, U_fc, X_array[:,:,index], X_tr_array[:,0,index], Xan_array[:,:,index], OI[:,index] = analysis_step_enkf_v5(U, U_tr_array, Y_obs, tmeasure, dtmeasure, index, PARS, h5_file_data, dirn)
            ...
            pool.terminate()
            pool.join()
            #raise ValueError('Failure in ensemble forecast')
            print(err)
...
def run_enkf_v2(i, j, m, l, U_tr_array, Y_obs, dirname, config_file, h5_file_data):
    ...
    # create readme file of exp summary and save
    PARS = [Nk_fc, Nk_tr, n_ens, assim_time, pars_enda, sig_ic, n_obs, NIAU, dres, sat_vel, n_obs_sat, n_obs_grnd, n_d, Neq, L, ob_noise, k, R, theta1, theta2, eta0, Z0, g, U_scale, sat_obs_mask, sigu_obs_mask, sigv_obs_mask, sigr_obs_mask, sig_c, sig_r, sat_init_pos, obs_T_d, obs_u_d, obd_v_d, obs_r_d, n_obs_T, n_obs_u, n_obs_v, n_obs_r, sigT_obs_mask]
    create_readme(dirn, config_file, i, j, m, l)
    ...
    while tmeasure-dtmeasure < t_end_assim and index < Nmeas:
        ...
        try:
            ...
            U_forec = np.zeros((Neq,Nk_fc,Nforec,n_ens))
            U_forec[:,:,0,:] = np.copy(U)

            print('Starting ensemble integrations from time =', assim_time[index],' to',assim_time[index+6]) 
            print('Number of cores used:', num_cores_use)
            print(' *** Started: ', str(datetime.now()))
       
            ### ADDITIVE INFLATION (moved to precede the forecast) ###
            q = add_inf[j] * np.random.multivariate_normal(np.zeros(n_d), Q, (n_ens,Nforec))
            q_ave = np.mean(q,axis=0)
            for N in range(n_ens):
                q[N,:,:] = q[N,:,:] - q_ave
                
                
            pool = mp.Pool(processes=num_cores_use,maxtasksperchild=1)
            mp_out = [pool.apply_async(forecast_step_isenRSW,args=(N,U_forec[:,:,:,N], U_rel, tau_rel, q[N,:,:].T, Neq, n_ens, n_d, Nk_fc, Kk_fc, cfl_fc, assim_time, index, assim_time[index], dtmeasure, Nforec, Mc, sig_c, sig_r, cc2, beta, alpha2, Ro, R, k, theta1, theta2, eta0, g, Z0, U_scale, Q, NIAU,h5_file_data)) for N in range(n_ens)] #<--- forecast step inside here of tunable length
            U_forec = [p.get() for p in mp_out]
            ...
            print(' All ensembles integrated forward from time =', assim_time[index],' to',assim_time[index+1])
            print(' *** Ended: ', str(datetime.now()))
            print(np.shape(U_forec))

            U_forec = np.swapaxes(U_forec,0,1)
            U_forec = np.swapaxes(U_forec,1,2)
            U_forec = np.swapaxes(U_forec,2,3)
            ...
            for N in range(n_ens):
                for T in range(Nforec):
                    X_forec[:,N,index,T] = U_forec_tmp[:,:,T,N].flatten()
            ...
            U_an, U_fc, X_array[:,:,index], X_tr_array[:,0,index], Xan_array[:,:,index], OI[:,index] = analysis_step_enkf_v4(U, U_tr_array, Y_obs, tmeasure, dtmeasure, index, PARS, h5_file_data)
